 ...
 * forwarded to the local Raspberry Pi Ollama cluster. No external AI provider
 * is contacted.
 *
 * Environment variables:
 *   OLLAMA_BASE_URL       — Ollama server base URL on Pi  (default: http://localhost:11434)
 *   OLLAMA_MODEL          — Model to use                  (default: llama3)
 *   OLLAMA_ALLOWED_ORIGIN — Allowed CORS origin           (default: http://localhost:11434)
 */

import { LOCAL_HANDLES, isLocalRequest, stripHandles } from '../src/ai/router.js';
...
export async function onRequestPost(context) {
  ...
  if (!isLocalRequest(message)) {
    return new Response(
      JSON.stringify({
        routed: false,
        hint: `Include one of ${LOCAL_HANDLES.join(', ')} to route to local Pi cluster.`,
      }),
      { status: 200, headers }
    );
  }
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ error: `Could not reach Pi cluster at ${baseUrl}: ${err.message}` }),
      { status: 502, headers }
    );
  }
  ...
  if (!ollamaRes.ok) {
    return new Response(
      JSON.stringify({ error: `Pi cluster responded with HTTP ${ollamaRes.status}` }),
      { status: 502, headers }
    );
  }
